...
「建 **[!UICONTROL 立表單]** 」標籤會列出您可在架構表單中使用的項目。 「設 **[!UICONTROL 定]** 」標籤提供您在「建置表單」標籤中選取之每 **[!UICONTROL 個項目的屬性]** 。 下表列出了「生成表單」頁籤中可 **[!UICONTROL 用的表單]** 項：
...
-- `./jcr:created`: Displays the jcr property at the asset&#39;s node. 如果您在檢視屬性上設定這些屬性，建議您將它們標示為「停用編輯」，因為這些屬性受到保護。否則，當您儲存資產的屬性時，會出現「資產無法修改」錯誤。
...
* 要刪除頁籤，請按一下 **[!UICONTROL x]**。 Click **[!UICONTROL Save]** to save the changes.

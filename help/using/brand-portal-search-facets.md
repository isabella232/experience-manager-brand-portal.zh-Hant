...
1. In the [!UICONTROL Edit Search Form] page, drag a predicate from the [!UICONTROL Select Predicate] tab to the main pane. 例如，拖曳 **[!UICONTROL Property Predicate]**。
...
1. 在「屬 **[!UICONTROL 性名稱]** 」欄位中，開啟屬性選擇器並選擇執行搜索的屬性。 或者，輸入屬性的名稱。 例如，輸入 `  jcr :content/metadata/dc:title` 或 `./jcr:content/metadata/dc:title`。
...
| **謂詞名稱** | **說明** | **屬性** |
|-------|-------|----------|
| **[!UICONTROL 路徑瀏覽器]** | 搜尋謂詞以搜尋特定位置的資產。 **注意：** *對於登入的使用者，「篩選」上的路徑瀏覽器只會顯示與使用者共用之資料夾（及其祖先）的內容結構。* <br> 管理員使用者可使用路徑瀏覽器導覽至任何資料夾，以搜尋該資產。 <br> 但是，非管理員使用者可在路徑瀏覽器中導覽至該資料夾，以搜尋資料夾中的資產（可供他們存取）。 | <ul><li>欄位標籤</li><li>路徑</li><li>說明</li></ul> |
| **[!UICONTROL 屬性]** | 根據特定中繼資料屬性搜尋資產。 **注意：** *在選取「部分搜尋」時，預設會選取「忽略大小寫」*。 | <ul><li>欄位標籤</li><li>預留位置</li><li>屬性名稱</li><li>部分搜尋</li><li>忽略大小寫</li><li> 說明</li></ul> |
...
| **[!UICONTROL 核准狀態]** | 搜尋謂詞以根據核准中繼資料屬性搜尋資產。 預設屬性名稱 **為dam:status**。 | <ul><li>欄位標籤</li><li>屬性名稱</li><li>說明</li></ul> |
...
1. 若要刪除屬性謂語，請按一下bin圖示。 在「刪 **[!UICONTROL 除欄位]** 」對話方塊中，按一 **[!UICONTROL 下「刪除]** 」以確認刪除動作。
   ...
   The **[!UICONTROL Property Predicate]** field is removed from the main pane, and the **[!UICONTROL Settings]** tab bemen empty.
